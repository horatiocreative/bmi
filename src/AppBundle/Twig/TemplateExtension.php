...
class TemplateExtension extends \Twig_Extension
{
    ...
    public function getFilters()
    {
        return array(
            ...
            'getInvestmentLifecycleStage' => new \Twig_SimpleFilter('get_investment_lifecyclestage', array($this, 'getInvestmentLifecycleStage')),
            'getOfferingLifecycleStage' => new \Twig_SimpleFilter('get_offering_lifecyclestage', array($this, 'getOfferingLifecycleStage'))
        );
    }
    ...
    public function getInvestmentLifecycleStage($life_cycle_stage)
    {
        if ($life_cycle_stage == 0) {
        	return 'Open';
        }
        else if ($life_cycle_stage == 1) {
        	return 'Rejected';
        }
        else if ($life_cycle_stage == 2) {
        	return 'Approved';
        }
        else if ($life_cycle_stage == 3) {
        	return 'Withdrawn';
        }
        else if ($life_cycle_stage == 4) {
        	return 'Settled';
        }                                
    ...
    public function getOfferingLifecycleStage($life_cycle_stage)
    {
        if ($life_cycle_stage == 0) {
        	return 'Draft';
        }
        ...
        else if ($life_cycle_stage == 5) {
        	return 'Published';
        }   
        else if ($life_cycle_stage == 6) {
        	return 'Live';
        }   
        else if ($life_cycle_stage == 7) {
        	return 'Closing';
        }   
        else if ($life_cycle_stage == 8) {
        	return 'Settled';
        }                                   
        else if ($life_cycle_stage == 9) {
        	return 'Cancelled';
        }       
        else {
        	return 'Unknown';
